...
import { buildAuthCallbackUrl, sanitizeAuthRedirectPath } from '../lib/auth';
import './Auth.css';
...
const Login: React.FC = () => {
  ...
  const redirectPath = useMemo(() => {
    ...
    return sanitizeAuthRedirectPath(state?.from);
  }, [location.state]);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      await signInWithGoogle(buildAuthCallbackUrl(redirectPath));
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Google ile giriş başlatılamadı.';
      setLocalError(message);
  ...
  return (
    <div className="auth-page">
      <div className="auth-container">
        ...
        <button className="google-btn" type="button" onClick={handleGoogleSignIn} disabled={isGoogleSubmitting}>
